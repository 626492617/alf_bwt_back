...
<body>
    ...
    <%-- var ftpurl = '<%=res.getString("QNURL")%>'; --%>
      ...
      //修改站点省
        ...
        /****************对战类型选择************************/
/*        	function addProjectIsCombat(project){
        	$("#raceiscombat").val(project);
        } */
        
        
        
        /**********************导入数据开始**************************************/
/*         function DelImportData(){
        	$("#tbFightTypeDialog").dialog("open");
        	FightTypeDataInit();
        }
        //数据列表
        function FightTypeDataInit() {
            $('#tbFightType').datagrid({
                url: "match_selectAllLsType.do",//访问路径
                width: '100%',
                height: $(parent.document).find("#mainPanel").height() - 100 > 0 ? $(parent.document).find("#mainPanel").height() - 150 : 500, //高度
                nowrap: true,
                striped: true,
                ...
                columns: [[
                    { field: 'cbx', checkbox: true },
                    { title: '类型名称', field: 'lstypename', width: 300, align: 'center' },
                    {
                        title: '类型描述', field: 'lsdescribe', width: 300, align: 'center', formatter: function (value) {
                    ...
                    {
                        title: '操作', field: 'typeid', width: 200, align: 'center', formatter: function (value, rec) {
                            return '<a class="a_edit" href="javascript:;" onclick="EditImportData(' + value + ');$(this).parent().click();return false;">选择选入的数据</a>';
                        }
                    }
                ]],
                pagination: true,
...
/*         function  EditImportData(typeid){
        	$("#typeid").val("");
        	$("#typeid").val(typeid);
        	$("#tbFightChooseEvents").dialog("open");
        	$('#tbChooseEvents').datagrid({
                url: "match_apiSelectAllMatch.do",//访问路径
                width: '100%',
                //height: '98%',
                nowrap: true,
                striped: true,
                ...
                columns: [[
                    { field: 'cbx', checkbox: true },
                    { title: '赛事名称', field: 'matchname', width: 300, align: 'center' },
                    { title: '赛书简称', field: 'matchabbreviation', width: 300, align: 'center' },
                    {
                        title: '图标', field: 'logo', width: 200, align: 'center', formatter: function (value) {
                            return "<img src='" + value + "' style='width:45px; height:45px;' />";
                        }
                    },
                    { title: '开始时间', field: 'gamedate', width: 300, align: 'center' },
                    { title: '结束时间', field: 'gameenddate', width: 300, align: 'center' }
                ]],
                toolbar: [
                          {
                              id: 'btnadd',
 ...
 /*        function AddDataImport(typeid){
        	var selected = "";
            $($('#tbChooseEvents').datagrid('getSelections')).each(function () {
                selected += this.remotedata + ",";
            });
            selected = selected.substr(0, selected.length - 1);
            if (selected == "") {
                $.messager.alert('提示', '请选择要导入的数据！', 'info');
                return;
            }else{
            	$.post("match_AddDataGameImportAPI.do", "remotedata="+selected+"&typeid="+typeid, function (msg) {
                    ReloadClearData();
                    if (msg != "0") {
                        $.messager.alert('提示', '导入成功', 'info');
                        $("#tbFightChooseEvents").dialog("close");
                        $("#tbFightTypeDialog").dialog("close");
                    }
                    else {
        ...
        //生成模板
/*         function EditDataTemplate(id){
        	 $.post("match_addTemplateData.do","matchid="+id, function(data){
                 ReloadClearData();
                 if (data == "1") {
                     $.messager.alert('提示', '操作成功', 'info');
                 }
